...
from X2CT.GAN.lib.utils.visualizer import tensor_back_to_unnormalization, tensor_back_to_unMinMax
from X2CT.GAN.lib.utils.metrics_np import MAE, MSE, Peak_Signal_to_Noise_Rate, Structural_Similarity, Cosine_Similarity, \
...
def parse_args_multi_view_cvpr():
  ...
  parse.add_argument('--check_point', type=str, default='90', dest='check_point',
  ...
  parse.add_argument('--load_path', type=str, default='../../CVPR_dataset/save_models/multiView_CTGAN/LIDC256/d2_multiview2500/checkpoint', dest='load_path',
...
def parse_args_single_view_cvpr():
  parse = argparse.ArgumentParser(description='CTGAN')
  parse.add_argument('--data', type=str, default='', dest='data',
                     help='input data ')
  parse.add_argument('--tag', type=str, default='', dest='tag',
  ...
  parse.add_argument('--model_class', type=str, default='SingleViewCTGAN', dest='model_class',
                     help='Model class should select from cyclegan / ')
  parse.add_argument('--check_point', type=str, default='30', dest='check_point',
                     help='which epoch to load? ')
  parse.add_argument('--latest', action='store_true', dest='latest',
                     help='set to latest to use latest cached model')
  parse.add_argument('--verbose', action='store_true', dest='verbose',
                     help='if specified, print more debugging information')
  parse.add_argument('--load_path', type=str, default='../../CVPR_dataset/save_models/singleView_CTGAN/LIDC256/d2_singleview2500/checkpoint', dest='load_path',
                     help='if load_path is not None, model will load from load_path')
  parse.add_argument('--how_many', type=int, dest='how_many', default=50,
                     help='if specified, only run this number of test samples for visualization')
  parse.add_argument('--resultdir', type=str, default='', dest='resultdir',
...
def parse_args_encoder_decoder_fully_attention():
  parse = argparse.ArgumentParser(description='CTGAN')
  parse.add_argument('--data', type=str, default='', dest='data',
                     help='input data ')
  parse.add_argument('--tag', type=str, default='', dest='tag',
  ...
  parse.add_argument('--datasetfile', type=str, default='./data/test.txt', dest='datasetfile',
                     help='Train or test or valid file path')
  parse.add_argument('--ymlpath', type=str, default='./test.yml', dest='ymlpath',# './test.yml'  './experiment/singleview2500/d2_singleview2500.yml'
                     help='config have been modified')
  parse.add_argument('--gpu', type=str, default='0', dest='gpuid',
                     help='gpu is split by ,')
  ...
  parse.add_argument('--verbose', action='store_true', dest='verbose',
                     help='if specified, print more debugging information')
  parse.add_argument('--load_path', type=str, default='./save_models/singleView_ED3D_sft/checkpoint', dest='load_path', #'./save_models/singleView_ED3D/checkpoint'
                     help='if load_path is not None, model will load from load_path')
  parse.add_argument('--resultdir', type=str, default='', dest='resultdir',
                     help='dir to save result')
  args = parse.parse_args()
...
def parse_args_CTGAN_fully_attention():
  parse = argparse.ArgumentParser(description='CTGAN')
  parse.add_argument('--data', type=str, default='', dest='data',
                     help='input data ')
  parse.add_argument('--tag', type=str, default='', dest='tag',
  ...
  parse.add_argument('--datasetfile', type=str, default='./data/test.txt', dest='datasetfile',
                     help='Train or test or valid file path')
  parse.add_argument('--ymlpath', type=str, default='./test.yml', dest='ymlpath',
                     help='config have been modified')
  parse.add_argument('--gpu', type=str, default='0', dest='gpuid',
                     help='gpu is split by ,')
  parse.add_argument('--dataset_class', type=str, default='align_ct_xray_std', dest='dataset_class',
                     help='Dataset class should select from unalign /')
  parse.add_argument('--model_class', type=str, default='SingleViewEncoderDecoder', dest='model_class', #SingleViewEncoderDecoder  SingleViewCTGAN
                     help='Model class should select from cyclegan / ')
  parse.add_argument('--check_point', type=str, default='90', dest='check_point',
                     help='which epoch to load? ')
  parse.add_argument('--latest', action='store_true', dest='latest',
  ...
  parse.add_argument('--verbose', action='store_true', dest='verbose',
                     help='if specified, print more debugging information')
  parse.add_argument('--load_path', type=str, default='./save_models/singleView_CTGAN/checkpoint', dest='load_path', #'./save_models/singleView_ED3D/checkpoint'  './save_models/singleView_ED3D_sft/checkpoint'
                     help='if load_path is not None, model will load from load_path')
  parse.add_argument('--resultdir', type=str, default='', dest='resultdir',
                     help='dir to save result')
  args = parse.parse_args()
...
def parse_args_Multi_ed_sft():
  ...
  parse.add_argument('--dataset_class', type=str, default='align_ct_xray_views', dest='dataset_class',
                     help='Dataset class should select from unalign /')
  parse.add_argument('--model_class', type=str, default='MultiViewEncoderDecoder', dest='model_class', #SingleViewEncoderDecoder  SingleViewCTGAN
                     help='Model class should select from cyclegan / ')
  parse.add_argument('--check_point', type=str, default='74', dest='check_point',
                     help='which epoch to load? ')
  parse.add_argument('--latest', action='store_true', dest='latest',
                     help='set to latest to use latest cached model')
  parse.add_argument('--verbose', action='store_true', dest='verbose',
                     help='if specified, print more debugging information')
  parse.add_argument('--load_path', type=str, default='./save_models/multiView_ED3D/checkpoint', dest='load_path', #'./save_models/singleView_ED3D/checkpoint'  './save_models/singleView_ED3D_sft/checkpoint'
                     help='if load_path is not None, model will load from load_path')
  parse.add_argument('--resultdir', type=str, default='', dest='resultdir',
                     help='dir to save result')
  args = parse.parse_args()
...
def evaluate(args):
  ...
  for epoch_i, data in tqdm.tqdm(enumerate(dataloader)):
    ...
    # elif 'views' in opt.dataset_class:
    #   generate_CT_transpose = generate_CT
    ...
    # print(np.max(generate_CT_transpose), np.max(real_CT_transpose))
    ...
    ssim = Structural_Similarity(real_CT_transpose, generate_CT_transpose, size_average=False, PIXEL_MAX=4096/2500)
    psnr_3d = Peak_Signal_to_Noise_Rate_3D(real_CT_transpose, generate_CT_transpose, size_average=False, PIXEL_MAX=4096/2500)
    # CT range 0-4096
    ...
    # print(data[2][0].split('\\')[1], data[2][0])
    import SimpleITK as sitk
    gen = sitk.GetImageFromArray(generate_CT_transpose.squeeze(0))
    sitk.WriteImage(gen, result_dir + '/' + data[2][0].split('\\')[1]+'_gen.nii.gz')

    real = sitk.GetImageFromArray(real_CT_transpose.squeeze(0))
    sitk.WriteImage(real, result_dir + '/' + data[2][0].split('\\')[1]+'_real.nii.gz')

    # psnr_3d = Peak_Signal_to_Noise_Rate_3D(real_CT_transpose, generate_CT_transpose, size_average=False, PIXEL_MAX=4096)
    psnr = Peak_Signal_to_Noise_Rate(real_CT_transpose, generate_CT_transpose, size_average=False, PIXEL_MAX=4096)
...
if __name__ == '__main__':
  # args = parse_args_multi_view_cvpr()
  # args = parse_args_single_view_cvpr()
  # args = parse_args_encoder_decoder_fully_attention()
  args = parse_args_Multi_ed_sft()
  evaluate(args)
